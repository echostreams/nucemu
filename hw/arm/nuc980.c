 ...
 *---------------------------------------------------------------------------*/
static void fmiSM_CorrectData_BCH(NUC970FmiState *fmi, uint8_t ucFieidIndex, uint8_t ucErrorCnt, uint8_t* pDAddr)
{
...
static int fmiPageSize(NUC970FmiState* fmi)
{
...
static int fmiSMCorrectData(NUC970FmiState *fmi, unsigned long uDAddr)
{
...
static void nuc970_init(MachineState* machine)
{
    ...
    */
    ...
    nuc970_uart_create(UART0_BA, 64, 0, serial_hd(0),
        qdev_get_gpio_in(aic, IRQ_UART0));
    //uart[1] = 
    nuc970_uart_create(UART1_BA, 64, 1, serial_hd(1),
        qdev_get_gpio_in(aic, IRQ_UART1));

    nuc970_uart_create(UART4_BA, 64, 4, serial_hd(4), qdev_get_gpio_in(aic, IRQ_UART4));
    nuc970_uart_create(UART5_BA, 64, 5, serial_hd(5), qdev_get_gpio_in(aic, IRQ_UART5));
    nuc970_uart_create(UART6_BA, 64, 6, serial_hd(6), qdev_get_gpio_in(aic, IRQ_UART6));
    nuc970_uart_create(UART7_BA, 64, 7, serial_hd(7), qdev_get_gpio_in(aic, IRQ_UART7));
    ...
    sysbus_connect_irq(s, 0, qdev_get_gpio_in(aic, IRQ_FMI));
     ...
     */
    sysbus_connect_irq(s, 0, qdev_get_gpio_in(aic, IRQ_EMC0_TX));
    sysbus_connect_irq(s, 1, qdev_get_gpio_in(aic, IRQ_EMC0_RX));
    ...
    */
    ...
    sysbus_connect_irq(s, 0, qdev_get_gpio_in(aic, IRQ_SDH));
    ...
    create_unimplemented_device("nuc970.pdma0", PDMA0_BA, 0x1000);
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(dev), 0, qdev_get_gpio_in(aic, IRQ_UDC));
     
    //create_unimplemented_device("nuc970.ohci", USBO_BA, 0x1000);
    create_unimplemented_device("nuc970.i2s", I2S_BA, 0x1000);
...

    {
        ...
        */
        ...
        object_property_set_bool(OBJECT(dev), "companion-enable",
            true, &error_fatal);
        sysbus_realize(SYS_BUS_DEVICE(dev), &error_fatal);
        sysbus_mmio_map(SYS_BUS_DEVICE(dev), 0, EHCI_BA);
        sysbus_connect_irq(SYS_BUS_DEVICE(dev), 0,
            qdev_get_gpio_in(aic, IRQ_EHCI));
        ...
        sysbus_realize(SYS_BUS_DEVICE(dev), &error_fatal);
        sysbus_mmio_map(SYS_BUS_DEVICE(dev), 0, OHCI_BA);
        sysbus_connect_irq(SYS_BUS_DEVICE(dev), 0, qdev_get_gpio_in(aic, IRQ_OHCI));
        ...
        sysbus_mmio_map(SYS_BUS_DEVICE(opmoden), 0, OHCI_BA + 0x204);
    ...
    info_report("Loading NUC980 kernel...\n");
}

static void nuc980_machine_init(MachineClass* mc)
{
...
DEFINE_MACHINE("nuc980", nuc980_machine_init)
